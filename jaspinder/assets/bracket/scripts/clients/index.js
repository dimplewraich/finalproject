gl.client = gl.client || {};
gl.client.listing = gl.client.listing || {
	...
	_init_d : function (){
		...
		var $this = $("#client_data_table").closest(".grid-list").find("form[name='grid-list-param']") 
				,g = $this.data('options');
		...
		g.grid = $.extend({
			...
			_default : true,
			_table_id : "#client_data_table",
			_aoColumnDefs : aoColumnDefs,
			_grid_url : "clients/getTable"
		}, g.grid,  {_frm : $this});
	...
	load_grid : function(g){
		...
		//var company_id = $("select#companySelect option:selected").sval();
		...
		gl.log_it("AFTER INIT CLIENTS DATA TABLE", $(g.grid._frm).data('options'));
	},
	
	close_grid : function (g){
	
		gl.log_it("CLOSE CLIENTS GRID", $(g.grid._frm).data('options'));
...
$('document').ready(function() {
	...
	if( typeof _grid_default_load !== 'undefined') gl.client.listing._default_load = _grid_default_load;
	if(gl.client.listing._default_load) gl.client.listing._init_m();
});