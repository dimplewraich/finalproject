gl.contact = gl.contact || {};
gl.contact.listing = gl.contact.listing || {
	
	_default_load : true,
	_init_m : function(){

		gl.contact.listing._init_d();
	...
	_init_d : function (){
		...
		var $this = $("#contact_data_table").closest(".grid-list").find("form[name='grid-list-param']") 
				,g = $this.data('options');
	...
	load_grid : function(g){
		...
		gl.log_it("AFTER INIT contact DATA TABLE", $(g.grid._frm).data('options'));
	},
	
	close_grid : function (g){
	
		gl.log_it("CLOSE contact GRID", $(g.grid._frm).data('options'));
...
$('document').ready(function() {
	...
	if( typeof _grid_default_load !== 'undefined') gl.contact.listing._default_load = _grid_default_load;
	
	if(gl.contact.listing._default_load) gl.contact.listing._init_m();
});